...
public class PageTable{
     ...
     */
    public PageTable(String method){
    ...
    public int get(int p_num){
        ...
        if(frameNumber == -1){
        	...
        	if(method == "LRU") {
            	...
            	recent.removeElement(frameNumber);
            	recent.push(frameNumber);
        	}
        }
        return frameNumber;
    }
    ...
    /**
     * function to remove updated page table
     *
     * @param f_num int Frame number
     */
    public void remove(int f_num) {
    	for(int i=0;i<this.table.length;i++) {
    		if(this.table[i].getFrameNumber()==f_num) {
...
class PageTableItem{
     ...
     * 
     * before return the frameNumber check the valid bit
     */
